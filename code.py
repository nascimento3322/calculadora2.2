print("Está é uma calculadora")

#Soma
a = int(input("Digite um número: "))
b = int(input("Digite mais um número: "))
soma = a + b
print(f"O resultado final dessa soma é {soma}")

#Subtração 
c = int(input("Digite um número para efetuar a subtração: "))
d = int(input("Digite um segundo valor: "))
subtração = c - d
print(f"O resultado final da subtração é: {subtração}")

#Multiplicação
e = int(input("Digite um número para efetuar a multiplicação: "))
f = int(input("Digite um segundo valor: "))
multiplicação = e * f
print(f"O resultado final da subtração é: {multiplicação}")

#Divisão
g = int(input("Digite um número para efetuar a divisão: "))
h = int(input("Digite um segundo valor: "))
divisão = g / h
print(f"O resultado final da subtração é: {divisão}")


